...
* **Change the maximum refinement depth (map layers):** edit **line 173** (`max_layer`)
 ...
 performance: for the test event (kb240697) which has 1095 data points, where we use ~33000 (logs,logq,logrho) maps, 16 alpha inital guesses, and 300+500 step MCMC for each (logs,logq,logrho,alpha), ~1 hour is needed on a 96-core node (this part should be highlight)

 plot the result of grid search
 run plot_grid_result.py, change event name and grid search result name at line 13 and 15, change max shown delta_chi2 at line 17, keep the hot_mcmc_factor at line 18 consistent with what you use; the resulting figure is saved into ./result/, and change line 180 for showing parameters of best xx solutions, column 0-7 means chi2/t0/u0/tE/alpha(degree)/logs/logq/logrho
...
* **Flux vs mag handling:** if using flux, repeat the file name at **line 299** (as currently implemented)
...
* MCMC `300 + 500` steps per `(logs, logq, logrho, alpha)`
...
To control how many best solutions are annotated in the plot:

* Edit **line 180** (number of best solutions shown)

Result table columns (0–7):
